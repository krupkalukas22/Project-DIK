...
// Data calculations with useMemo
                ...
                <>
                  {files.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {files.map(file => (
                        <div key={file.id} onClick={() => handleSelectFile(file)} className={`p-4 rounded-xl cursor-pointer transition-all duration-300 transform hover:scale-[1.02] ${selectedFile?.id === file.id ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-2xl scale-[1.02]' : darkMode ? 'bg-gray-700/50 hover:bg-gray-600/50 text-gray-200 border border-gray-600' : 'bg-white/50 hover:bg-white border border-gray-200 text-gray-700 shadow-lg hover:shadow-xl'}`}>
                          <div className="flex items-start justify-between mb-2">
                            <p className="font-semibold text-sm break-words">{file.name}</p>
                            <button onClick={(e) => { e.stopPropagation(); handleDeleteFile(file.id); }} className={`ml-2 p-1 rounded hover:bg-red-500 hover:text-white transition ${selectedFile?.id === file.id ? 'text-white' : 'text-gray-400'}`}>
                              <Trash2 size={16} />
                            </button>
                          </div>
                ...
                {showSheets && (
                  <div className="space-y-2">
                    {selectedFile.sheetNames.map((sheet) => (
                      <button key={sheet} onClick={() => loadSheet(selectedFile.workbook, sheet)} className={`w-full text-left px-4 py-3 rounded-xl transition-all duration-300 transform hover:scale-[1.02] font-semibold ${selectedSheet === sheet ? 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg' : darkMode ? 'bg-gray-700/50 text-gray-200 hover:bg-gray-600/50 border border-gray-600' : 'bg-white/50 text-gray-700 hover:bg-white shadow hover:shadow-lg border border-gray-200'}`}>
                        {sheet}
                      </button>
                    ))}
                  </div>
                ...
                <div className={`${darkMode ? 'bg-gray-800/50 backdrop-blur-xl border border-gray-700' : 'bg-white/70 backdrop-blur-xl border border-white'} rounded-2xl shadow-2xl p-6 transition-all duration-300`}>
                  ...
                  {showDashboard && (
                    <>
                              ...
                              <div className="flex items-center gap-3 mb-3">
                                {item.showMF ? (
                                  <div className="relative">
                                    <div className={`p-2 rounded-xl ${darkMode ? 'bg-white/10' : 'bg-gray-100'}`}>
                      ...
                      {extremeDays.most && (
                        <div className="mb-6">
                          <div className={`${darkMode ? 'bg-gradient-to-r from-green-900/50 to-emerald-800/50 border border-green-600/30' : 'bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200'} rounded-2xl p-5 backdrop-blur-sm shadow-xl`}>
                            <p className={`text-sm font-bold ${darkMode ? 'text-green-300' : 'text-green-700'} mb-2`}>⚡ Největší zátěž v období</p>
                            <p className={`text-2xl font-black ${darkMode ? 'text-green-100' : 'text-green-600'}`}>{extremeDays.most.datum}</p>
                            <p className={`text-sm ${darkMode ? 'text-green-300' : 'text-green-600'} mt-2 font-semibold`}>Celkem: {extremeDays.most.total}</p>
                          </div>
                        </div>
                      )}
                          ...
                          <thead>
                            <tr className={`${darkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gradient-to-r from-indigo-50 to-purple-50 border-gray-300'} border-b-2`}>
                              {[
                                ...
                                <th key={col.key} onClick={() => sortTable(col.key)} className={`px-4 py-3 text-left font-black ${darkMode ? 'text-gray-200 hover:bg-gray-600' : 'text-gray-700 hover:bg-indigo-100'} cursor-pointer transition-all duration-300`}>
...
<td className={`${darkMode ? 'text-gray-300' : 'text-gray-700'} px-4 py-3`}>
  {row['D'] || '-'}
</td>
<td className={`${darkMode ? 'text-gray-300' : 'text-gray-700'} px-4 py-3`}>
  {row['E'] || '-'}
